...
[root@Jenkins-HI-Class-VM:/home/sigongweb/work/db_mon]cat slave01_check.sh 
#!/bin/sh
...
if [ "$Slave_IO_Running" = "Yes" -a "$Slave_SQL_Running" = "Yes" ]; then
        ...
        echo "<tr style='background: crimson; text-align: center;' >"
                ...
                echo "<td> $Last_SQL_Error </td>"
        echo "</tr>"
echo "http://10.1.10.5:19999/db_check.html" | mail -s "$IPADDR DB 서버의 Replication이 실패했습니다. 긴급 점검하세요." kisssulran@i-screammedia.com
echo "http://10.1.10.5:19999/db_check.html" | mail -s "$IPADDR DB 서버의 Replication이 실패했습니다. 긴급 점검하세요." tipa3246@i-screammedia.com
fi
